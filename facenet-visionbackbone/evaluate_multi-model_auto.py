...
def extractlistfea(filelist, model, feadict):
    for item_path in tqdm(filelist, desc='extract fea'):
        try:
            ...
            features = model.extract_imagefea(image_db)
...
def get_args_parser():
    ...
    parser.add_argument('--modelpaths', nargs='*', default=['/home/dongqing/coinAI-main/algorithm/coinrecogination/facenet-pytorch/ep398-loss0.007-val_loss0.042.pth', '/home/dongqing/coinAI-main/algorithm/coinrecogination/facenet-pytorch/ep398-loss0.007-val_loss0.042.pth'])
    return parser


def eval_single_model(modelpath, db_dir, query_dir): 
    extractor = None
    extractor = initExtractorFacenet(modelpath)
    ...
    querypop_dict = dict()
    querymatch_dict = dict()
    querynum_dict = dict()

    for query in tqdm(queryfea_dict.keys(), desc='eval'):
        cointype = query.split('-')[0]
        if cointype in querynum_dict.keys():
            querynum_dict[cointype] += 1
        else:
            querynum_dict[cointype] = 1 
        ...
        if top1_type == top2_type:
            #top1等于top2,pop+1
            pop_num += 1
            if cointype in querypop_dict.keys():
                querypop_dict[cointype] += 1
            else:
                querypop_dict[cointype] = 1
        ...
        if match_tag == True:
            match_num += 1
            if cointype in querymatch_dict.keys():
                querymatch_dict[cointype] += 1
            else:
                querymatch_dict[cointype] = 1
    ...
    return querynum_dict, querypop_dict, querymatch_dict
...
if __name__ == "__main__":
    ...
    wbres = Workbook()
    ws = wbres.active
    ws.append(['模型文件名', '版别ID', 'query图片数', 'match数', 'pop数'])

    for modelpath in args.modelpaths:
        querynum_dict, querypop_dict, querymatch_dict = eval_single_model(modelpath, db_dir, query_dir)
        for cointype in querynum_dict.keys():
            if cointype in querymatch_dict.keys():
                matchnum = querymatch_dict[cointype]
            else:
                matchnum = 0

            if cointype in querypop_dict.keys():
                popnum = querypop_dict[cointype]
            else:
                popnum = 0
            ws.append([modelpath, cointype, querynum_dict[cointype], matchnum, popnum])
